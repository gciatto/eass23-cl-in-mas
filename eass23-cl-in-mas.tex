...
\begin{frame}{\currentExercise{} -- Logic solvers and side-effects \hfill (advanced, \alert{optional})}
    ...
    \begin{block}{Goal}
        ...
        \begin{description}\small
            \item[\pl{not(P)}] which fails if predicate \pl{P} is true
            \item[\pl{assert(C)}] which adds clause \pl{C} to the solver's knowledge base
            \item[\pl{retract(C)}] which removes clause \pl{C} from the solver's knowledge base
            \item[\pl{is(X, E)}] which assigns to variable \pl{X} the value attained by evaluating expression \pl{E}
        \end{description}
    \end{block}
\end{frame}
